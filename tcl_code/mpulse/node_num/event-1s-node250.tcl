...
set val(nn) 251 ;# number of mobilenodes
set val(rp) MPULSE ;# routing protocol
...
set tracefd [open tr-mp-250.tr w]
$ns_ trace-all $tracefd

set namtrace [open nam-mp-250.nam w]
...
for {set y 0} {$y < 10} {incr y} {
	set posY [expr {($y * 100.0) + 230.0}]
	for {set x 0} {$x < 25} {incr x} {
		set posX [expr { $x * 160.0 }]
...
$app_($num) set rate_ 8kbps
$app_($num) set packetSize_ 100
...
# event generate interval = 1 s
# 10 packets per event
#set stopTime 0
#for {set i 0.2} {$i < $val(stop)} {set i [expr $stopTime + 1.0]} {
